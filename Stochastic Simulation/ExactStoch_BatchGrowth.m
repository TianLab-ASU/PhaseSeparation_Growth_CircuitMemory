function [t,X, N_dilute, N_droplet,concentration_dilute,concentration_droplet,concentration_ave,vt_rec]=ExactStoch_BatchGrowth(DefineReactions,tspan, IC)
...
while (t(end)<maxT && any(X(end,:)) ) 
    ...
    if abs(timeafterdivision-DoublingTime)<DoublingTime/100 %cell division
        ...
        Vt=Vt/2; 
        CellNum=CellNum*2;
        GrowthRATE=growthrate*(1-CellNum/Nmax1)*(t(end)>lagphase);   
        DoublingTime=log(2)/GrowthRATE; 
    end
 

    [S, P, K, N_dilute_SS, N_droplet_SS] = DefineReactions(t(end),X(end,:)); % defines reaction stoichiometry

    N_dilute(end+1)=N_dilute_SS;
    N_droplet(end+1)=N_droplet_SS; 
 
    vt_rec(end+1)=Vt;
    V_droplet=N_droplet_SS*V_TF;
    V_dilute=Vt-V_droplet; 
    concentration_dilute(end+1)=N_dilute_SS/V_dilute/Omega;
    concentration_droplet(end+1)=N_droplet_SS/V_droplet/Omega*Drop_Factor;
    concentration_ave(end+1,:)=X(end,:)/Vt/Omega; 
